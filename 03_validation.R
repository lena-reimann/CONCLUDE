...
# pop
t = (1990, 2000, 2010)
pop_rur_med <- stack(paste0("rur_pop", t, ".tif"))
pop_urb_med <- stack(paste0("urb_pop", t, ".tif"))  
pop_tot_med <- stack(paste0("tot_pop", t, ".tif"))

# alphas
alpha00_u <- raster(paste0(paste("P1_alpha", "u", sep = "_"), ".tif"))
alpha00_r <- raster(paste0(paste("P1_alpha", "r", sep = "_"), ".tif"))

alpha10_u <- raster(paste0(paste("P2_alpha", "u", sep = "_"), ".tif"))
alpha10_r <- raster(paste0(paste("P2_alpha", "r", sep = "_"), ".tif"))
...
write.csv(err_df, paste0(paste(paste(path, task, "err_esti", sep = "/"), reg, "cntr", model_period, p, sep = "_"), ".csv"), row.names = F)
...
writeRaster(tot_reg, filename=paste(paste(paste(path, task, reg, sep = "/"), "pop_tot", t, p, sep = "_"), "tif", 
                                    sep = "."), format="GTiff", bylayer = T, overwrite=TRUE)
writeRaster(urb_reg, filename=paste(paste(paste(path, task, reg, sep = "/"), "pop_urb", t, p, sep = "_"), "tif", 
                                    sep = "."), format="GTiff", bylayer = T, overwrite=TRUE)
writeRaster(rur_reg, filename=paste(paste(paste(path, task, reg, sep = "/"), "pop_rur", t, p, sep = "_"), "tif", 
                                    sep = "."), format="GTiff", bylayer = T, overwrite=TRUE)