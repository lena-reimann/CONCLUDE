...
# comments: # 1) alpha_r set to 0 where urb90 > 0
...
path <- "G:/PhD/Fulbright/Demographic_model/model_extension/calibration/slurm_NEW"
#setwd("G:/PhD/Fulbright/Demographic_model/model_extension/")

# load data needed for processing
load(paste(paste(paste(path, task, sep = "/"), "input", sep = "_"), "RData", sep = "."))

# set path again because in the data loaded above the path is defined differently
path <- "G:/PhD/Fulbright/Demographic_model/model_extension/calibration/slurm_NEW"
...
# resample data to align them
#pop_rur <- resample(pop_rur, mask00_med, method= "ngb")
#pop_urb <- resample(pop_urb, mask00_med, method= "ngb")
#pop_tot <- resample(pop_tot, mask00_med, method= "ngb")
...
for (i in 1:length(id)) {
  ...
  dist_m <- readMM(paste(paste("Theo/dist_m", iso[i], sep = "/"), "mtx.gz", sep = "."))
  
  ## (1) select mask for cntr
  mask00_c <- mask00[[which(ids == id[i])]]
  mask_c  <- mask[[which(ids == id[i])]]
  ...
  # write csvs
  write.csv(alpha_r, paste0(paste(paste(path, "chunks", iso[i], sep = "/"), "alpha_r", p, b, sep = "_"), ".csv"), 
            row.names = F)
  write.csv(alpha_u, paste0(paste(paste(path, "chunks", iso[i], sep = "/"), "alpha_u", p, b, sep = "_"), ".csv"), 
            row.names = F)
...
## make mosaic of scaled alphas for entire region
...
# write raster
# path <- "E:/model_extension/calibration/slurm_NEW/chunks/results" # for writing rasters on laptop
#writeRaster(alpha_u_c, paste0(paste(paste(path, folder, folder, sep = "/"), p, "alpha", "u", sep = "_"), ".tif"))
#writeRaster(alpha_r_c, paste0(paste(paste(path, folder, folder, sep = "/"), p, "alpha", "r", sep = "_"), ".tif"))


#RData
save.image(paste(paste(paste(path, task, sep = "/"), p, b, sep = "_"), "RData", sep = "."), 
           version = 2)
