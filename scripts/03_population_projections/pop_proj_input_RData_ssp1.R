...
# comments: # 1) for calculation on rz cluster: two scripts --> (1) data prep; (2) parallel processing
            # 2) for entire region
            # 3) use readAll() function for including all raster info in the workspace when saving it 
...
#--------------------------------------------------------------
# (1) load mask data 
# for ssp1: leave out the year 2010 (because we start projection for 2020 and the 2010 mask already accounts for SLR)

#entire med
mask00_med <- readAll(raster("spatial_mask/final/sp_mask_ssp1v5.tif"))
...
# (5b) load distance raster for reducing the number of grid cells of large countries
# -------------------------------------
...
# (6) load inundation layers for each time step for retreat assumptions (SSP1 only)
#--------------------------------------------------------------
inun_nA <- list()
inun_wA <- list()
for (i in seq(t2)) {  
  ...
  name <- paste0(paste(paste0("inundation/results/inun30_sl/inun30_sl26"), t[i], sep = "_"), ".tif")
  inun_nA[[i]] <- readAll(raster(name))
  
  ## wA
  #load inun for each t (years 2010 to 2100) including retreat assumptions (# we do not need the permanent inun layer because already part of the retreat layer)
  name <- paste0(paste(paste0("inundation/results/inun30_esl_wA/inun30_esl26_wA_rp_r"), t[i], sep = "_"), ".tif")
  inun_wA[[i]] <- readAll(raster(name))
}

save.image(paste(paste(paste(task, task, sep = "/"), "input", ssp,  sep = "_"), "RData", sep = ".")) #v,
