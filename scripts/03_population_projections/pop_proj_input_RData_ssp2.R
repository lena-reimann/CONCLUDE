...
# comments: # 1) for calculation on rz cluster: two scripts --> (1) data prep; (2) parallel processing
            # 2) for entire region EXCLUDING DZA
...
#load masks per country for each t (we use mask of ssp3 for ssp2 as well)
for (i in seq(ids)) {
  name <- paste0(paste(paste0(paste(paste0("spatial_mask/sp_mask_cntr25_ssp3/sp_mask"), "ssp3_id", sep = "_"), ids[i]), 
                       t_sp_mask, sep = "_"), ".tif")  
...
# (6) load inundation layers for each time step for locations permanently inundated due to SLR (not here because we don't use ssp2 for migration paper)
#--------------------------------------------------------------
#inun_nA <- list()
#inun_wA <- list()
...
#  name <- paste0(paste(paste0("inundation/results/inun30_sl/inun30_sl45"), t[i], sep = "_"), ".tif")
#  inun_nA[[i]] <- readAll(raster(name))
  
  ## wA
  #load inun for each t (years 2010 to 2100)
#  name <- paste0(paste(paste0("inundation/results/inun30_sl_wA/inun30_sl45_wA"), t[i], sep = "_"), ".tif")
